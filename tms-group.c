/*首先执行编译命令：gcc tms-group.c -o tms-group -lpthread
 *然后运行生成的.out程序命令：./tms-group
*/
...
//	printf("file_buf1=%s\n",file_buf1);
	...
	if(same_file2 == NULL)
	{
		printf("Please check the radio USB.\n");
		return (1);
	}

	/*增加目标地址*/
	snprintf(buf3 , sizeof(buf3) , "225.0.0.0");
...
//	printf("file_buf2=%s\n",file_buf2);
	...
	same_file3=strstr(file_buf2,buf3);
	//添加接收群发信息的目标地址12.0.0.0命令
	if(same_file1)
	{
//		printf("same=%s\n",same_file1);
		printf("Destination ip 12.0.0.0 was already added.\n");
	}
	else
	{
//		printf("command:'sudo route add -net 12.0.0.0 netmask 255.0.0.0 gw 192.168.6.1'\n");
	...
	//添加群发的目标地址225.0.0.0命令
	if(same_file3)
	{
//		printf("same_file=%s\n",same_file3);
		printf("Destination ip 225.0.0.0 was already added.\n");
		return (0);
	}
	else
	{
//		printf("command:'sudo route add -net 225.0.0.0 netmask 255.0.0.0 gw 192.168.6.1'\n");
		snprintf(command2 , sizeof(command2) , "sudo route add -net 225.0.0.0 netmask 255.0.0.0 gw 192.168.6.1");
		if(NULL == (fp4 = popen(command2 , "r")))
			printf("fp4 failed\n");
	}
...
int client_thread()
{
	...
	server.sin_addr.s_addr = inet_addr("225.0.3.232");//群发的目标地址
...
int main()
{
	...
	if(run_prepared()==1)//判断对讲机USB是否插好以及目标地址是否添加
		return 1;
